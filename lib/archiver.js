...
function Archiver(root, store) {
	...
	this.mirror = function(feed) {
		var dirname = feed.url.replace('dat://', '')
			.replace(/\//g, '')

		if (feed.http && feed.http.port && typeof feed.http.port == 'number') {
			console.log(feed.url + ' is mirrored to HTTP on port ' + feed.http.port)
			
			this.dats[dirname].serveHttp({
				port: feed.http.port,
				live: true
			})
		}
